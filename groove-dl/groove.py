...
def retrieveSongUrl(query):
    print entrystring #Print the welcome message
    print 'Initializing...'
    getToken() #Get a static token
    #query = ' '.join(query) #Get the search parameter
    print ('Searching for "' + query + '"...').encode('utf-8')
    m = 0
    s = getResultsFromSearch(query) #Get the result from the search
    ...
    print "Enter the Song ID you wish to download or (q) to exit: "
    songid = '1'
    print songid
    ...
    songurl = 'http://%s/stream.php?streamKey=%s' % (stream["ip"], stream["streamKey"])
    artist =  s[songid]["ArtistName"]
    title =  s[songid]["SongName"]
    return songurl, artist, title
